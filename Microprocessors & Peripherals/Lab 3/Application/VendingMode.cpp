...
void VendingMode::Inner::startReleaseWorkflow()
{
  ...
  VendingScreen::drawReleaseMessage();
	...
	u16 change = 0;
	change = mCashBox.getMoneyInIntermediateCash() - mProductSlots[mSelectedProduct].getPricePerItem();
	VendingScreen::drawChangeInfo( mProductSlots[mSelectedProduct].getPricePerItem(), change );
